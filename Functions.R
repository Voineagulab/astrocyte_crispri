...
geneInfo <- read.table(sep = "\t", file = "/mnt/Data0/PROJECTS/CROPSeq/Pilot_Reanalysis/Data/GeneInfo.txt", header = TRUE)
...
find.nearby.tss <- function(query = example.query, expand.by = 100000, info = geneInfo, reduced.output = TRUE) {
  ...
  # collect 
  within.window <- info[which(info$Chr == chr & (info$Start > window.start) & (info$Start < window.end)),]
...
## Lift hg19 to hg38
  ...
  chainFile <- "/mnt/Data0/PROJECTS/CROPSeq/PublicData/hg19ToHg38.over.chain"
  
  lift.bed <- function(chain = chainFile, bed.in.19, bed.out.38, isList = TRUE) {
    ...
    export(y@unlistData, bed.out.38)
