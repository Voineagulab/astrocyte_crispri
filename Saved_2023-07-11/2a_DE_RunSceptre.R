...
## Pretty simple to run for target genes...
  a <- Sys.time()
...
## Now run transcriptome-wide
  ## Guide-gene pairs
    exp.thresh <- 2^-6
    exp.mean <- rowMeans(nha@assays$RNA@data[, sceptre.use]) 
    exp.high <- names(exp.mean)[exp.mean > exp.thresh]
    exp.high <- c(exp.high, sceptre.pairs.pos$gene_id) %>% unique()
    
    sceptre.pairs.pos.tw <- data.frame(gene_id = rep(exp.high, each = length(targ.pos)), # tw stands for "transcriptome-wide"
                                       gRNA_id = targ.pos,
                                       pair_type = "positive_control")
    
    save(sceptre.pairs.pos.tw, file = "Pos/Transcriptome-wide Pairs - Input.rda")

  ## Setup loop
    # but just for ID3!
    # sceptre.pairs.pos.tw <- sceptre.pairs.pos.tw[sceptre.pairs.pos.tw$gRNA_id == "ID3",]
    de.pos.tw <- list()
    chunkSize <- 250 # just to ensure there's > 1 guide in the matrix...
    nChunks <- ceiling(nrow(sceptre.pairs.pos.tw) / chunkSize)
    save.points <- seq(0, nChunks, 100)

  ## Run loop (requires 1-2min per chunk on our server)
    # for (j in 1:nChunks) {
    for (j in (length(de.pos.tw)+1):nChunks) {
      ...
      chunk <- chunk[which(chunk < nrow(sceptre.pairs.pos.tw))] # truncate if the range is outside the full dataframe
      chunk <- sceptre.pairs.pos.tw[chunk,]

      # run sceptre
      de.pos.tw[[run.name]] <- run_sceptre_high_moi(gene_matrix = sceptre.exp[chunk$gene_id,],
                                                  combined_perturbation_matrix = sceptre.guide.pooled, 
                                                  covariate_matrix = sceptre.covar,
      ...
      print(paste0(j, ": ", b-a))
      if (j %in% save.points) save(de.pos.tw, file = paste0("Temp Pos (Chunk_", j, ").rda"))
      gc()
    }


  ## Save results
    de.pos.tw <- do.call("rbind", de.pos.tw)
    save(de.pos.tw, file =  "Pos/Transcriptome-wide - SCEPTRE Output.rda")
...
# ## Using the 250 negative controls in the Neg transduction pool, paired
#     sceptre.pairs.neg <- list()
#     run.scale.factor <- 3
#     n <- ceiling(length(used.genes) / run.scale.factor)
#     for (j in 1:n) { # this chunks sceptre in runs with 125 guide pairs for 100 genes
#       range <- ((run.scale.factor*(j-1)) + 1) : (run.scale.factor*j) # get 100 genes
...
# 
#     # run
#     save.points <- names(sceptre.pairs.neg)[seq(0, 1000, 100)]
#     de.neg.pooled <- list()
...
#       # run sceptre
#       a <- Sys.time()
#       de.neg.pooled[[j]] <- run_sceptre_high_moi(gene_matrix = sceptre.exp[unique(k$gene_id),],
#                                                combined_perturbation_matrix = sceptre.guide.pooled,
#                                                covariate_matrix = sceptre.covar,
...
# 
#       print(paste0(j, ": ", b-a))
#       if (j %in% save.points) save(de.neg.pooled, file = paste0("Temp Neg Pooled (", j, ").rda"))
#       gc()
#     }
# 
# 
#   ## Save results
#     de.neg.pooled <- do.call("rbind", de.neg.pooled)
#     save(de.neg.pooled, file =  "Neg/SCEPTRE Output (Neg, Pooled).rda")
