...
## Run DropletQC to identify empty and/or damaged cells
  dqc <- list()
  for (j in samples) {
    dqc[[j]] <- nuclear_fraction_tags(outs = paste0("../../Data/Sequencing/FinalCount/", j, "/outs/"),
                                      # barcodes = gsub(paste0(j, "_"), "", colnames(nha)[which(nha$Library == j)]),
                                      tiles = 1, cores = 30, verbose = TRUE)
    gc()
  }
  
  # there is a missing barcode for NHA_8: GTAACACGTAGTATAG-1. Run manually.
  barc <- "GTAACACGTAGTATAG-1"
  missed <- nuclear_fraction_tags(bam = ("../../Data/Sequencing/FinalCount/NHA_8/outs/possorted_genome_bam.bam"),
                                   bam_index = ("../../Data/Sequencing/FinalCount/NHA_8/outs/possorted_genome_bam.bam.bai"),
                                   barcodes = c(barc, colnames(obj$NHA_8)[1:4]), # dummy in the second index, otherwise it thinks this is a filename...
                                   tiles = 1, cores = 30, verbose = TRUE)
  
  dqc$NHA_8[barc,] <- missed[barc,]
  
  # save
  save(dqc, file = "Library-level/DropletQC.rda")
  
  # process
  dqc.processed <- list()
  for (lib in samples) {
    # extract
    x <- data.frame(NucFrac = dqc[[lib]][colnames(obj[[lib]]),],
                    UMI = obj[[lib]]$nCount_RNA,
                    row.names = colnames(obj[[lib]]))
    
    # identify empty drops
    pdf(file = paste0("Library-level/Droplet QC - ", lib, ".pdf"), height = 5, width = 8)
    y <- identify_empty_drops(x, include_plot = TRUE)
    dev.off()
    
    # identify damaged cells
    y$Celltype <- "NHA"
    z <- identify_damaged_cells(y)
    
    dqc.processed[[lib]] <- z$df
  ...
  for (j in samples) {
    # m <- match(rownames(obj[[j]]@meta.data), rownames(dqc.processed[[j]])) # unecessary to match
    obj[[j]]@meta.data$NuclearFraction <- dqc.processed[[j]]$NucFrac
    obj[[j]]@meta.data$DropletQC <- dqc.processed[[j]]$cell_status
  }
...
## Run basic UMAP 
  ...
  umap.plot("G2M.Score")
  umap.plot("G2M.Score")
  umap.plot("NuclearFraction")
  umap.plot("DropletQC") + scale_colour_carto_d(palette = "Geyser") 
...
## Recalculate metadata that change depending on relative properties
  ...
  library()
  x <- scuttle::normalizeCounts(as.matrix(nha@assays$RNA@counts)) # per recommendations, use this function
  ...
  y <- project_cycle_space(x = x,
                           species = "human",
                           gname.type = "SYMBOL")
  nha$Tricycle.PC1 <- y[,1]
  nha$Tricycle.PC2 <- y[,2]
...
## Redo columnames
  colnames(nha@meta.data) <- c("Library", "UMI_Total", "Gene_Total", 
                               ...
                               "NuclearFraction", "DropletQC",
                               "Cluster",
                               "Cycle_Tricycle", "Cycle_Tricycle_Phase", "Cycle_Tricycle_PC1", "Cycle_Tricycle_PC2", "Cycle_Schwabe_Phase")
...
## Plot UMAP
  ...
  pdf(file = "Pooled/UMAP - Cell Cycle (Continuous).pdf", height = 12, width = 10)
  ...
  pdf(file = "Pooled/UMAP - Cell Cycle (Phases).pdf", height = maxh, width = maxw)
...
## Propose cells to be cut
  ...
  r <- nha$Ribo_Pct > 20 | nha$MALAT1_Pct > 20
  table(r, nha$DropletQC)
...
## Apply threshold and recluster
  # apply threshold
  keep <- which(nha$Ribo_Pct < 20 & nha$MALAT1_Pct < 20 & nha$DropletQC != "damaged_cell")
