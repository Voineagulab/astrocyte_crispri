...
## Read in
sy <- read.csv("Results/SY5Y/Selected Guides.csv")
  
## Step 2: create informative columns
  ## An enhancer id
    sy$TargetID <- factor(sy$Input)
    levels(sy$TargetID) <- paste0("Enh", 1:length(levels(sy$TargetID)))
    
   ## An enhancer region tag
    sy$TargetCoord <- sy$Target.Alias
  ...
  ## Guide coordinates
    ...
    x <- sapply(strsplit(x, ":") , "[", 2)
    x <- as.numeric(x)
    x <- x + sy$sgRNA.Cut.Position..1.based. # start point of the sgRNA
    
    z <- rep(1, nrow(sy))
    z[sy$Orientation == "antisense"] <- -1
    z <- z * 20 # end point of the sgRNA, accounting for strand!
    
    sy$GuideCoord <- paste0(y, ":", x, "-", (x + z))
    
  ## Guide Sequence
    sy$GuideSequence <- sy$sgRNA.Sequence # just renaming
    
  ## Guide ID
    y <- x <- as.character(sy$TargetID) # get the target id of every guide
    for (j in 1:length(x)) {
      z <- x[1:j-1] 
      z <- length(which(z == x[j])) # counts how many guides earlier in the list have the same target
      z <- z + 1 # add 1 to this number
    ...
    sy$GuideID <- paste0(y, "_", sy$GuideCoord)
    
  ## Get relevant columns
    sy <- sy[,c("GuideID", "GuideSequence", "GuideCoord", "TargetCat", "TargetID", "TargetCoord")]
  
## Add positive controls
  sy <- rbind(sy, enh.pos)
    
## Export
  write.csv(sy, file = "Results/Final_List/SY5Y Enh Library.csv", row.names = FALSE)
...
# not being made, will be requested from Sefi